...
class GETNOVELPROTOTYPEHook(Hook):
    ...
    def _get_prototype(self, runner):
        ...
        split_name = data_loader.dataset.ann_cfg[0]['setting'].split('_')[0]
        base_cls_num = len(data_loader.dataset.SPLIT['BASE_CLASSES_'+split_name])
        novel_cls_num = len(data_loader.dataset.SPLIT['NOVEL_CLASSES_'+split_name])
        all_roi_features = torch.zeros(novel_cls_num, data_loader.dataset.num_novel_shots,
                                       1024).cuda()
        ...
        num_bbox_saved_per_cls = np.zeros(novel_cls_num, np.int8)  ###保存当前已保存每类目标的数目
            ...
            with torch.no_grad():
                ...
                if bbox_feats.shape[0] > 0:
                    for nn in range(bbox_feats.shape[0]):
                        ...
                        if label >= base_cls_num: ###novel label编号在base类别编号之后
                            label = label - base_cls_num
