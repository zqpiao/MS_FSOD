...
class FewShotVOCDataset(BaseFewShotDataset):
    ...
    def __init__(self,
        ...
        # if is_ann_shot_filter and ann_shot_filter is None:
...
class FewShotVOCDefaultDataset(FewShotVOCDataset):
    ...
    voc_benchmark = {
        f'SPLIT{split}_SEED{seed}_{shot}SHOT': [
            dict(
                type='ann_file',
                ann_file=f'data/few_shot_ann/voc/seed/seed{seed}/'
                         f'box_{shot}shot_{class_name}_train.txt',

                # ann_file=f'/data/piaozhengquan/projects/FSOD/mmfewshot-main/data/few_shot_ann/voc/seed/seed{seed}/'
                #          f'box_{shot}shot_{class_name}_train.txt',

                ann_classes=[class_name])
            for class_name in VOC_SPLIT[f'ALL_CLASSES_SPLIT{split}']
        ]
        for shot in [1, 2, 3, 5, 10] for seed in list(range(10)) for split in [1, 2, 3]
    }
