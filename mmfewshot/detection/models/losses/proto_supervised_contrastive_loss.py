...
class SupervisedContrastiveLoss(nn.Module):
    ...
    def forward(self,
        ...
        ##-----------prototype_loss----------
        x_dot_proto = torch.mm(features, prototypes.t())
        labels = labels.reshape(-1)
        keep_proto = ious[labels < prototypes.shape[0]] >= 0.5
        # xx = x_dot_proto[labels < prototypes.shape[0]][keep_proto]
        # ll = labels[labels<prototypes.shape[0]][keep_proto]
        loss_proto = self.ce_loss(x_dot_proto[labels<prototypes.shape[0]][keep_proto], labels[labels<prototypes.shape[0]][keep_proto])
