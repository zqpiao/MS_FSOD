...
class CosineSimBBoxHead_PROTO_AVG(ConvFCBBoxHead):
    ...
    def forward(self, x: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        cls_score_src = self.scale * self.fc_cls(
        ...
        logit_proto_x = torch.zeros((x_cls.shape[0], self.num_classes + 1), dtype=torch.float).cuda()

        ####CosineSimilarity
        # logit_proto_x = cls_score.new_tensor(cls_score)
        if not isinstance(prototype_per_cls_gt, list):  ##非空
            # prototype_per_cls_gt = prototype_per_cls_gt.t()
            # cls_score = torch.zeros((x_cls.shape[0], prototype_per_cls_gt.shape + 1), dtype=torch.float).cuda()

            # logit_proto_x = torch.zeros((x_cls.shape[0], prototype_per_cls_gt.shape[0] + 1), dtype=torch.float).cuda()
            # logit_proto_x[:,:-1] = self.scale * self.CosineSimilarity(x_cls, prototype_per_cls_gt) ##损失函数为交叉熵

            logit_proto_x = self.scale * self.CosineSimilarity(x_cls, prototype_per_cls_gt) ##损失函数为交叉熵


            # cls_score[:, :cls_scores.shape[1]] = cls_scores
            # softmax_proto_x = self.soft_max(self.scale * cls_score)

            # print('cls_score_cossim: ', cls_score_src)
            # print('cls_scores_proto: ', cls_score)
            # print('softmax_proto_x: ', softmax_proto_x)
        ...
        if not self.use_proto_avr_score:
            return cls_score_src, bbox_pred, x_cls
        else:
            return cls_score_src, bbox_pred, x_cls, logit_proto_x
    ...
    def loss(self,
        ...
        ###----------prototypr_x cos sim margin --------
        ####margin cross_entropy loss
        # --------------------------- convert label to one-hot ---------------------------
        margin = 0.2
        num_base_cls = 0
        if logit_proto_x.shape[1] == 5: ##VOC
            num_base_cls = 15
            num_all_cls = 20
        sel = (labels >= num_base_cls) & (labels < num_all_cls)
        logit_proto_x = logit_proto_x[sel]
        label_novel = labels[sel] - num_base_cls  ###novel label转为0——num_novel_cls -1

        # labels[sel] = labels[sel] - num_base_cls  ###novel label转为0——num_novel_cls -1
        # labels[~sel] = logit_proto_x.shape[1] - 1   ###其他类均置为最后一个label

        if label_novel.shape[0] > 0:
            one_hot = torch.zeros_like(logit_proto_x).scatter_(1, label_novel.view(-1, 1), 1)

            logit_proto_x = logit_proto_x - one_hot * margin
            #
            # # # element-wise losses
            loss_ = F.cross_entropy(
                logit_proto_x,
                label_novel,
                # weight=torch.tensor(0.5).cuda(),
                # weight=logdiff,
                reduction='mean',)
        ...
        loss_weight = 0.1
        losses['loss_cls_proto_novel'] = loss_weight * loss_

        # if logit_proto_x is not None:
        #     avg_factor = max(torch.sum(label_weights > 0).float().item(), 1.)
        #     if logit_proto_x.numel() > 0:
        #         loss_cls_proto_x = self.loss_proto_novel(
        #             logit_proto_x,
        #             labels,
        #             label_weights,
        #             avg_factor=avg_factor,
        #             reduction_override=reduction_override)
        #         if isinstance(loss_cls_proto_x, dict):
        #             losses.update(loss_cls_proto_x)
        #         else:
        #             losses['loss_cls_proto_x'] = loss_cls_proto_x
    ...
    def get_bboxes(self,
        ...
        if self.custom_cls_channels:
            ...
            # if not isinstance(logit_proto_x, list):
            #     # sf = F.softmax(logit_proto_x, dim=-1)
            #     scores[:, -logit_proto_x.shape[1]:] = alpha * scores[:, -logit_proto_x.shape[1]:] + (1 - alpha) * F.softmax(logit_proto_x, dim=-1)
