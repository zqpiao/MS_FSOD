...
from mmfewshot.classification.models.heads import RelationHead


def test_meta_baseline_head():
    head = RelationHead(in_channels=64)
    support_feat = torch.randn(4, 64, 7, 7)
    query_feat = torch.randn(4, 64, 7, 7)
    ...
    head = RelationHead(
        in_channels=64, loss=dict(type='MSELoss', loss_weight=1.0))
    losses = head.forward_train(support_feat, label, query_feat, label)
    assert losses['loss'].item() > 0
