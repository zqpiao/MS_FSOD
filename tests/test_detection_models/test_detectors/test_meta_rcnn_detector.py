...
def test_meta_rcnn_detector_fp16_forward(cfg_file):
    if not torch.cuda.is_available():
        import pytest
        pytest.skip('test requires GPU and torch+cuda')
    model = _get_detector_cfg(cfg_file)
    ...
    support_data = dict(
        ...
        img=torch.randn(4, 4, 320, 320).cuda(),
        gt_bboxes=copy.deepcopy(gt_bboxes) * 4,
        gt_labels=[
            torch.LongTensor([0]).cuda(),
            torch.LongTensor([1]).cuda(),
            torch.LongTensor([2]).cuda(),
            torch.LongTensor([3]).cuda()
        ],
    )
    model.train()
    losses = model(
    ...
    data_init = dict(
        ...
        img=torch.randn(5, 4, 320, 320).cuda(),
        gt_bboxes=copy.deepcopy(gt_bboxes) * 5,
        gt_labels=[torch.LongTensor([i]).cuda() for i in range(5)],
    )
    with torch.no_grad():
        model(**data_init, mode='model_init')
        results = model(
